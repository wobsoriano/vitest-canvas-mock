...
export default class CanvasRenderingContext2D {
  ...
  clearRect(x, y, width, height) {
    ...
    if (!Number.isFinite(x + y + width + height))
      return
  ...
  drawImage(img, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight) {
    ...
    if (arguments.length === 3) {
      if (!Number.isFinite(sxResult + syResult))
        return
      sx = 0
      sy = 0
      sWidth = img.width
  ...
  fillText(text, x, y, maxWidth) {
    ...
    if (arguments.length === 3 && !Number.isFinite(xResult + yResult))
      return
    if (
      arguments.length > 3
  ...
  quadraticCurveTo(cpx, cpy, x, y) {
    ...
    if (!Number.isFinite(cpxResult + cpyResult + xResult + yResult))
      return
  ...
  setTransform(a, b, c, d, e, f) {
    ...
    e = Number(e)
    f = Number(f)
    if (!Number.isFinite(a + b + c + d + e + f))
      return
