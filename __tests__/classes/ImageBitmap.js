...
describe('image bitmaps', () => {
  ...
  it('should throw if width or height is not finite or 0', () => {
    return Promise.all([
      createImageBitmap(img, 1, 2, Number.NaN, 3).then(throwError).catch(expectRangeError),
      createImageBitmap(img, 1, 2, 0, 3).then(throwError).catch(expectRangeError),
      createImageBitmap(img, 1, 2, 3, Number.NaN).then(throwError).catch(expectRangeError),
  ...
  it('should throw if last parameter is not object if source rect is provided', () => {
    return Promise.all(
      ['', 0, Number.NaN, Infinity].map((e) =>
        createImageBitmap(img, 1, 2, 3, 4, e).then(throwError).catch(expectTypeError),
      ),
    );
  });
