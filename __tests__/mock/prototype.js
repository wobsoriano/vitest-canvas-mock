...
describe('mock', () => {
  ...
  it('should accept image/jpeg in toDataURL', () => {
    expect(canvas.toDataURL('image/jpeg')).toMatch(/image\/jpeg/);
  });

  it('should accept image/webp in toDataURL', () => {
  ...
  it('should work in both cases where passed in window object has and does not have .HTMLCanvasElement.prototype', () => {
    ...
    expect(vi.isMockFunction(mockWin.HTMLCanvasElement.prototype.getContext)).toBe(false);
    expect(vi.isMockFunction(mockWin.HTMLCanvasElement.prototype.toBlob)).toBe(false);
    expect(vi.isMockFunction(mockWin.HTMLCanvasElement.prototype.toDataURL)).toBe(false);

    // act
    mockPrototype(mockWin);
    // assert ( should mock out expected fields in passed in window object )
    expect(vi.isMockFunction(mockWin.HTMLCanvasElement.prototype.getContext)).toBe(true);
    expect(vi.isMockFunction(mockWin.HTMLCanvasElement.prototype.toBlob)).toBe(true);
    expect(vi.isMockFunction(mockWin.HTMLCanvasElement.prototype.toDataURL)).toBe(true);
  });
});
